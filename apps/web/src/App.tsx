...
import { AppDownloadPromo } from "./app/AppDownloadPromo";
...
export default function App() {
  ...
  const [appNavTab, setAppNavTab] = useState<"GAME" | "CHAT" | "WALLET" | "PROFILE">("GAME");
  ...
  const authRoute = ["/login", "/register", "/app/login", "/app/register"].includes(window.location.pathname);
  const authPageMode = window.location.pathname.endsWith("/register") ? "REGISTER" : "LOGIN";
  const referralCodeFromUrl = useMemo(() => {
    const queryCode = new URLSearchParams(window.location.search).get("ref")?.trim();
    const pathMatch = window.location.pathname.match(/^\/refer\/([^/]+)/i);
    return (pathMatch?.[1] || queryCode || "").trim().toUpperCase();
  }, []);
  ...
  useEffect(() => {
    const onCampaignNavigate = (event: Event) => {
      ...
      if (target === "BONUSES") { setBonusSection("LEVEL_UP"); setBonusOpen(true); }
      if (target === "REFERRAL") { setBonusSection("REFERRAL"); setBonusOpen(true); }
  ...
  const appActiveNav = profileCenterOpen ? "PROFILE" : financeOpen ? "WALLET" : chatOpen ? "CHAT" : appNavTab;
  const openAuthPage = (mode: "LOGIN" | "REGISTER") => { const authPath = appRoute ? mode === "REGISTER" ? "/app/register" : "/app/login" : mode === "REGISTER" ? "/register" : "/login"; window.location.assign(authPath); };
  const openAppAuth = () => openAuthPage("LOGIN");
  const openAppWallet = () => { setAppNavTab("WALLET"); if (!user) { openAppAuth(); return; } setAccountMode("REAL"); setFinanceTab("DEPOSIT"); setFinanceOpen(true); };
  const openAppProfile = () => { setAppNavTab("PROFILE"); if (!user) { openAppAuth(); return; } setProfileCenterOpen(true); setProfileMenuOpen(false); };
  const openAppGame = () => { setAppNavTab("GAME"); setChatOpen(false); setFinanceOpen(false); setProfileCenterOpen(false); setBonusOpen(false); window.scrollTo({ top: 0, behavior: "smooth" }); };
  ...
  if (!user && authRoute) {
    return <AuthPage initialMode={authPageMode} referralCode={referralCodeFromUrl} onAuthenticated={(authenticatedUser) => { setUser(authenticatedUser); window.location.replace(appRoute ? "/app/" : "/"); }} onBackToLanding={() => window.location.replace(appRoute ? "/app/" : "/")} />;
  }
  ...
  return <div className={`app-shell ${accountMode === "DEMO" ? "demo-mode" : ""} ${!appRoute ? "website-shell" : ""}`}>
    ...
    {!appRoute && <AppDownloadPromo />}
    <header className="topbar"><button className="back">‹</button><Logo /><div className="mode-switch"><button className={accountMode === "REAL" ? "active" : ""} onClick={() => setAccountMode("REAL")}>Real</button><button className={accountMode === "DEMO" ? "active" : ""} onClick={() => setAccountMode("DEMO")}>Demo</button></div><div className="top-actions">{user ? <>{accountMode === "DEMO" && <button onClick={() => void resetDemo()} disabled={demoResetBusy}>{demoResetBusy ? "Resetting…" : "Reset Demo"}</button>}<button className="header-icon-button notification-button" onClick={() => setNotificationsOpen(true)} aria-label="Notifications">🔔{notificationUnread > 0 && <span>{Math.min(notificationUnread, 99)}</span>}</button><div className="profile-wrap"><button className="profile" onClick={() => setProfileMenuOpen((value) => !value)}>{user.avatarUrl ? <img src={user.avatarUrl} alt="" /> : user.name.slice(0, 1).toUpperCase()}</button>{profileMenuOpen && <div className="profile-menu"><strong>{user.name}</strong><span>{user.email}</span><small>{user.authProvider === "GOOGLE" ? "Google account" : user.authProvider === "HYBRID" ? "Email + Google" : "Email account"}</small><button onClick={() => { setProfileCenterOpen(true); setProfileMenuOpen(false); }}>Profile & settings</button>{accountMode === "REAL" ? <button onClick={() => { setFinanceTab("DEPOSIT"); setFinanceOpen(true); setProfileMenuOpen(false); }}>Wallet & payments</button> : <button onClick={() => void resetDemo()}>Reset demo balance</button>}<button onClick={() => { setBonusSection("LEVEL_UP"); setBonusOpen(true); setProfileMenuOpen(false); }}>VIP bonuses</button><button onClick={() => { setBonusSection("REFERRAL"); setBonusOpen(true); setProfileMenuOpen(false); }}>Invite & earn</button><button onClick={() => { setFaqOpen(true); setProfileMenuOpen(false); }}>Help & FAQs</button><button className="danger-text" onClick={() => void logout()}>Sign out</button></div>}</div></> : <><button className="guest-auth-button" onClick={() => openAuthPage("LOGIN")}>Login</button><button className="guest-auth-button guest-auth-primary" onClick={() => openAuthPage("REGISTER")}>Sign up</button></>}</div></header>
    <div className="game-title-row"><div className="game-name">B9T9 {accountMode === "DEMO" && <span className="demo-mode-badge">DEMO</span>}</div><CampaignExperience placement="GAME" /><div className="balance">{accountMode === "REAL" && <span className={`connection ${connected ? "ok" : ""}`}>{statusLabel}</span>}<button className="balance-amount" type="button" onClick={() => { if (user) { setFinanceTab("DEPOSIT"); setFinanceOpen(true); } else { openAppAuth(); } }}><strong>{visibleBalance.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</strong> PKR</button>{accountMode === "REAL" && (wallet.lockedBalance > 0 || wallet.bettingLockedBalance > 0) && <span className="locked-balance">Locked {(wallet.lockedBalance + wallet.bettingLockedBalance).toLocaleString()} PKR</span>}<button className="toolbar-button chat-symbol" onClick={() => setChatOpen((value) => !value)}>◯</button></div></div>
    <main className={`game-layout ${chatOpen ? "chat-open" : ""}`}><BetsList bets={visibleBets} /><section className={`center-column ${historyExpanded ? "history-open" : ""}`}>{historyStrip}<GameGraph round={round} /><div className="bet-panels" data-app-bets><BetPanel slot="left" round={round} wallet={wallet} accountMode={accountMode} authenticated={Boolean(user)} onRequireAuth={openAppAuth} onNotify={notify} /><BetPanel slot="right" round={round} wallet={wallet} accountMode={accountMode} authenticated={Boolean(user)} onRequireAuth={openAppAuth} onNotify={notify} /></div></section><ChatPanel chat={chat} online={visibleOnline} onClose={() => setChatOpen(false)} user={user} onRequireAuth={openAppAuth} onNotify={notify} supportOpenRequest={supportOpenRequest} /></main>
    ...
    {financeOpen && user && accountMode === "REAL" && <FinanceModal wallet={wallet} initialTab={financeTab} onClose={() => setFinanceOpen(false)} onWalletRefresh={setWallet} />}{proofRoundId && <ProvablyFairModal roundId={proofRoundId} onClose={() => setProofRoundId(null)} />}{bonusOpen && user && <BonusCenter initialSection={bonusSection} onClose={() => setBonusOpen(false)} onNotify={notify} />}
    ...
    {appRoute && <AppBottomNav active={appActiveNav} loggedIn={Boolean(user)} onGame={openAppGame} onChat={() => { setAppNavTab("CHAT"); setChatOpen(true); }} onWallet={openAppWallet} onProfile={openAppProfile} />}
  </div>;
}
