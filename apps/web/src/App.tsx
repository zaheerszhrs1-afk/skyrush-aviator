...
export default function App() {
  ...
  return <div className={`app-shell ${accountMode === "DEMO" ? "demo-mode" : ""}`}>
    ...
    <div className="game-title-row"><div className="game-name">B9T9 {accountMode === "DEMO" && <span className="demo-mode-badge">DEMO</span>}</div><CampaignExperience placement="GAME" /><div className="balance">{accountMode === "REAL" && <span className={`connection ${connected ? "ok" : ""}`}>{statusLabel}</span>}<strong>{visibleBalance.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</strong> PKR{accountMode === "REAL" && (wallet.lockedBalance > 0 || wallet.bettingLockedBalance > 0) && <span className="locked-balance">Locked {(wallet.lockedBalance + wallet.bettingLockedBalance).toLocaleString()} PKR</span>}{accountMode === "REAL" && <button className="toolbar-button" onClick={() => setFinanceOpen(true)}>☰</button>}<button className="toolbar-button chat-symbol" onClick={() => setChatOpen((value) => !value)}>◯</button></div></div>
    <main className={`game-layout ${chatOpen ? "chat-open" : ""}`}><BetsList bets={visibleBets} online={visibleOnline} /><section className={`center-column ${historyExpanded ? "history-open" : ""}`}>{historyStrip}<GameGraph round={round} /><div className="bet-panels"><BetPanel slot="left" round={round} wallet={wallet} accountMode={accountMode} onNotify={notify} /><BetPanel slot="right" round={round} wallet={wallet} accountMode={accountMode} onNotify={notify} /></div></section><ChatPanel chat={chat} online={visibleOnline} onClose={() => setChatOpen(false)} user={user} onNotify={notify} supportOpenRequest={supportOpenRequest} /></main>
