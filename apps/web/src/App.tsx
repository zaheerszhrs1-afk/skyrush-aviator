...
export default function App() {
  ...
  const [chat, setChat] = useState<ChatItem[]>([]); const [connected, setConnected] = useState(false); const [chatOpen, setChatOpen] = useState(false); const [financeOpen, setFinanceOpen] = useState(false); const [bonusOpen, setBonusOpen] = useState(false);
  ...
  useEffect(() => {
    if (!user) { socket.disconnect(); setConnected(false); return; }
  ...
  return <div className={`app-shell ${accountMode === "DEMO" ? "demo-mode" : ""}`}>
    <CampaignExperience placement="GAME" />
    ...
    <header className="topbar"><button className="back">‹</button><Logo /><div className="mode-switch"><button className={accountMode === "REAL" ? "active" : ""} onClick={() => setAccountMode("REAL")}>Real</button><button className={accountMode === "DEMO" ? "active" : ""} onClick={() => setAccountMode("DEMO")}>Demo</button></div><div className="top-actions"><button className="vip-launch" onClick={() => setBonusOpen(true)}>VIP Bonuses</button>{accountMode === "REAL" ? <button onClick={() => setFinanceOpen(true)}>Add Cash</button> : <button onClick={() => void resetDemo()} disabled={demoResetBusy}>{demoResetBusy ? "Resetting…" : "Reset Demo"}</button>}<button className="header-icon-button" onClick={() => setFaqOpen(true)} aria-label="Help">?</button><button className="header-icon-button notification-button" onClick={() => setNotificationsOpen(true)} aria-label="Notifications">🔔{notificationUnread > 0 && <span>{Math.min(notificationUnread, 99)}</span>}</button><div className="profile-wrap"><button className="profile" onClick={() => setProfileMenuOpen((value) => !value)}>{user.avatarUrl ? <img src={user.avatarUrl} alt="" /> : user.name.slice(0, 1).toUpperCase()}</button>{profileMenuOpen && <div className="profile-menu"><strong>{user.name}</strong><span>{user.email}</span><small>{user.authProvider === "GOOGLE" ? "Google account" : user.authProvider === "HYBRID" ? "Email + Google" : "Email account"}</small><button onClick={() => { setProfileCenterOpen(true); setProfileMenuOpen(false); }}>Profile & settings</button>{accountMode === "REAL" ? <button onClick={() => setFinanceOpen(true)}>Wallet & payments</button> : <button onClick={() => void resetDemo()}>Reset demo balance</button>}<button onClick={() => { setBonusOpen(true); setProfileMenuOpen(false); }}>VIP bonuses</button><button onClick={() => { setFaqOpen(true); setProfileMenuOpen(false); }}>Help & FAQs</button><button className="danger-text" onClick={() => void logout()}>Sign out</button></div>}</div></div></header>
    <div className="game-title-row"><div className="game-name">B9T9 {accountMode === "DEMO" && <span className="demo-mode-badge">DEMO</span>}</div><div className="balance">{accountMode === "REAL" && <span className={`connection ${connected ? "ok" : ""}`}>{statusLabel}</span>}<strong>{visibleBalance.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</strong> PKR{accountMode === "REAL" && (wallet.lockedBalance > 0 || wallet.bettingLockedBalance > 0) && <span className="locked-balance">Locked {(wallet.lockedBalance + wallet.bettingLockedBalance).toLocaleString()} PKR</span>}{accountMode === "REAL" && <button className="toolbar-button" onClick={() => setFinanceOpen(true)}>☰</button>}<button className="toolbar-button chat-symbol" onClick={() => setChatOpen((value) => !value)}>◯</button></div></div>
    <main className={`game-layout ${chatOpen ? "chat-open" : ""}`}><BetsList bets={visibleBets} online={visibleOnline} /><section className={`center-column ${historyExpanded ? "history-open" : ""}`}>{historyStrip}<GameGraph round={round} /><div className="bet-panels"><BetPanel slot="left" round={round} wallet={wallet} accountMode={accountMode} onNotify={notify} /><BetPanel slot="right" round={round} wallet={wallet} accountMode={accountMode} onNotify={notify} /></div></section><ChatPanel chat={chat} online={visibleOnline} onClose={() => setChatOpen(false)} /></main>
    {!chatOpen && <button className="floating-chat" aria-label="Open public chat" onClick={() => setChatOpen(true)}>💬</button>}<SupportChat user={user} onNotify={notify} />
    {financeOpen && accountMode === "REAL" && <FinanceModal wallet={wallet} onClose={() => setFinanceOpen(false)} onWalletRefresh={setWallet} />}{proofRoundId && <ProvablyFairModal roundId={proofRoundId} onClose={() => setProofRoundId(null)} />}{bonusOpen && <BonusCenter onClose={() => setBonusOpen(false)} onNotify={notify} />}
    {profileCenterOpen && <ProfileCenter user={user} onClose={() => setProfileCenterOpen(false)} onUserUpdated={setUser} onNotify={notify} />}{faqOpen && <FaqCenter onClose={() => setFaqOpen(false)} />}{notificationsOpen && <NotificationsCenter onClose={() => setNotificationsOpen(false)} onUnreadChange={setNotificationUnread} />}
  </div>;
}
