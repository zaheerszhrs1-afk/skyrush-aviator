...
export default function App() {
  ...
  const [chat, setChat] = useState<ChatItem[]>([]); const [connected, setConnected] = useState(false); const [chatOpen, setChatOpen] = useState(false); const [financeOpen, setFinanceOpen] = useState(false); const [financeTab, setFinanceTab] = useState<"DEPOSIT" | "WITHDRAW" | "HISTORY">("DEPOSIT"); const [bonusOpen, setBonusOpen] = useState(false);
  ...
  return <div className={`app-shell ${accountMode === "DEMO" ? "demo-mode" : ""}`}>
    ...
    <main className={`game-layout ${chatOpen ? "chat-open" : ""}`}><BetsList bets={visibleBets} online={visibleOnline} /><section className={`center-column ${historyExpanded ? "history-open" : ""}`}>{historyStrip}<GameGraph round={round} /><div className="bet-panels"><BetPanel slot="left" round={round} wallet={wallet} accountMode={accountMode} onNotify={notify} /><BetPanel slot="right" round={round} wallet={wallet} accountMode={accountMode} onNotify={notify} /></div></section><ChatPanel chat={chat} online={visibleOnline} onClose={() => setChatOpen(false)} user={user} onNotify={notify} /></main>
    {!chatOpen && <button className="floating-chat" aria-label="Open live chat" onClick={() => setChatOpen(true)}>💬</button>}<WhatsAppBadge />
    {financeOpen && accountMode === "REAL" && <FinanceModal wallet={wallet} initialTab={financeTab} onClose={() => setFinanceOpen(false)} onWalletRefresh={setWallet} />}{proofRoundId && <ProvablyFairModal roundId={proofRoundId} onClose={() => setProofRoundId(null)} />}{bonusOpen && <BonusCenter onClose={() => setBonusOpen(false)} onNotify={notify} />}
