...
export default function App() {
  ...
  const [appNavTab, setAppNavTab] = useState<"GAME" | "BETS" | "CHAT" | "WALLET" | "PROFILE">("GAME");
  ...
  useEffect(() => {
    if (!user && referralCodeFromUrl) {
      setAuthStartMode("REGISTER");
      setGuestAuthOpen(true);
    }
  }, [referralCodeFromUrl, user]);
  ...
  const openAppAuth = () => { setAuthStartMode("LOGIN"); setGuestAuthOpen(true); };
  ...
  const openAppBets = () => { setAppNavTab("BETS"); setChatOpen(false); window.requestAnimationFrame(() => document.querySelector("[data-app-bets]")?.scrollIntoView({ behavior: "smooth", block: "center" })); };
  ...
  return <div className={`app-shell ${accountMode === "DEMO" ? "demo-mode" : ""}`}>
    ...
    {appRoute && <AppBottomNav active={appActiveNav as "GAME" | "BETS" | "CHAT" | "WALLET" | "PROFILE"} loggedIn={Boolean(user)} onGame={openAppGame} onBets={openAppBets} onChat={() => { setAppNavTab("CHAT"); setChatOpen(true); }} onWallet={openAppWallet} onProfile={openAppProfile} />}
  </div>;
}
