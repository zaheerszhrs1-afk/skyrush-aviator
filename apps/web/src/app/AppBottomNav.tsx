type AppNavKey = "GAME" | "BETS" | "CHAT" | "WALLET" | "PROFILE";
...
function NavIcon({ kind }: { kind: AppNavKey }) {
  const paths: Record<AppNavKey, string> = {
    ...
    BETS: "M4 5h16v14H4zM8 9h8M8 13h5",
...
export function AppBottomNav({ active, loggedIn, onGame, onBets, onChat, onWallet, onProfile }: AppBottomNavProps) {
  const items: Array<{ key: AppNavKey; label: string; onClick: () => void }> = [
    { key: "GAME", label: "Game", onClick: onGame },
    { key: "BETS", label: "Bets", onClick: onBets },
